...
const getBroadcastUsers = async (req, res) => {
  try {
    ...
    for (const user of targets) {
      ...
      if (!chatId) {
        console.log("⚠ No chat_id found, skipping user");
        continue; // ← was: return — now correctly skips just this user
      }
          ...
          inline_keyboard:
            msg.buttons?.map((btn) => [
              btn.type === "webapp"
                ...
                : btn.type === "callback"
                ? { text: btn.text, callback_data: btn.command || btn.data || btn.text }
...
const markBroadcastDone = async (req, res) => {
  try {
    ...
    const msg = await ScheduledMessage.findById(message);
    if (!msg) {
      // was: return — request hung forever with no response
    ...
    console.error("❌ markBroadcastDone error:", error);
