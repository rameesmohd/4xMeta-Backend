...
//     const managerRunningEquity = new Map();
...
//         compoundReturnToInc = toTwoDecimals(compounded * 100) - existingChart.value;
//         existingChart.value = toTwoDecimals(compounded * 100);
//         await existingChart.save({ session });
//       } else {
//         compoundReturnToInc = dailyReturn
...
const rollOverTradeDistribution = async (rollover_id) => {
  ...
  try {
    ...
    // Track running equity per manager
    const managerRunningEquity = new Map();
    
    // Track accumulated profit values per manager
    const managerAccumulated = new Map();
    
    // Track daily chart values per manager per day
    ...
    for (const trade of unDistributedTrades) {
      ...
      // Initialize running equity once per manager
      if (!managerRunningEquity.has(managerId)) {
        managerRunningEquity.set(managerId, Number(manager.total_funds) || 0);
      }

      // Initialize accumulated values
      if (!managerAccumulated.has(managerId)) {
        managerAccumulated.set(managerId, {
      ...
      // ------------------------------------------------------------
      // MANAGER GROWTH CHART CALCULATION
      // ------------------------------------------------------------
      const chartDate = dayjs(trade.close_time).startOf("day").toDate();
      const chartKey = `${managerId}_${chartDate.getTime()}`;

      // Load existing chart value once per day
      if (!dailyChartMap.has(chartKey)) {
        const existingChart = await managerGrowthChart
          .findOne({ manager: manager._id, date: chartDate })
          .session(session);

        const existingValue = existingChart ? Number(existingChart.value) : 0;
        dailyChartMap.set(chartKey, existingValue);
      }

      // Calculate return based on equity change
      const previousEquity = managerRunningEquity.get(managerId);
      const newEquity = previousEquity + tradeProfit;

      managerRunningEquity.set(managerId, newEquity);

      const dailyReturn = previousEquity > 0
        ? toTwoDecimals(((newEquity - previousEquity) / previousEquity) * 100)
        : 0;

      // Compound the returns for this day
      const currentChartValue = dailyChartMap.get(chartKey);
      const compounded = (1 + currentChartValue / 100) * (1 + dailyReturn / 100) - 1;
      const newCompoundedValue = toTwoDecimals(compounded * 100);

      dailyChartMap.set(chartKey, newCompoundedValue);

      // Update accumulated manager values
      const accumulated = managerAccumulated.get(managerId);
      accumulated.closed_trade_profit += tradeProfit;
      accumulated.total_trade_profit += tradeProfit;
      accumulated.total_funds += tradeProfit;
    ...
    // ------------------------------------------------------------
    // UPDATE MANAGER GROWTH CHARTS FIRST
    // ------------------------------------------------------------
    const chartBulk = [];

    for (const [key, value] of dailyChartMap.entries()) {
      const [managerId, ts] = key.split("_");
    ...
    if (chartBulk.length)
      await managerGrowthChart.bulkWrite(chartBulk, { session });

    // ------------------------------------------------------------
    // CALCULATE LIFETIME COMPOUND FROM CHART (✅ FIX ISSUE 1)
    // ------------------------------------------------------------
    const bulkManagerUpdates = [];
    ...
    for (const [managerId, accumulated] of managerAccumulated.entries()) {
      ...
      bulkManagerUpdates.push({
        updateOne: {
          filter: { _id: managerObjectId },
          update: {
            $inc: {
              closed_trade_profit: accumulated.closed_trade_profit,
              total_trade_profit: accumulated.total_trade_profit,
              total_funds: accumulated.total_funds,
            },
            $set: {
              compound: lifetimeCompound, // ✅ Now correct across all days
            },
          },
        },
      });
    ...
    // ------------------------------------------------------------
    // RECALCULATE MANAGER TOTAL RETURN
    // ------------------------------------------------------------
    const managerIds = Array.from(touchedManagers).map(id => new mongoose.Types.ObjectId(id));
    const managers = await managerModel
      .find({ _id: { $in: managerIds } })
      .session(session);
    ...
    for (const m of managers) {
      const deposit = Number(m.total_deposit) || 0;
      const profit = Number(m.total_trade_profit) || 0;

      const totalReturn = deposit > 0 ? toTwoDecimals((profit / deposit) * 100) : 0;
    ...
    // ------------------------------------------------------------
    // USER PORTFOLIO RETURN CHART (✅ FIX ISSUE 3)
    // ------------------------------------------------------------
    if (touchedUsers.size) {
      const userIds = Array.from(touchedUsers).map(
        (id) => new mongoose.Types.ObjectId(id)
      ...
      // ✅ Simply aggregate current totals per user
      const userStats = await investmentModel.aggregate([
        { $match: { user: { $in: userIds }, status: "active" } },
        {
          $group: {
      ...
      // ✅ One entry per user (not per trade!)
      for (const u of userStats) {
        const deposit = Number(u.totalDeposit) || 0;
        const profit = Number(u.totalTradeProfit) || 0;

        const totalReturn = deposit > 0 
          ? toTwoDecimals((profit / deposit) * 100) 
          : 0;

        userChartBulk.push({
          updateOne: {
            filter: { user: u._id, date: chartDate },
            update: { $set: { value: totalReturn } },
            upsert: true,
          },
        });
      }
